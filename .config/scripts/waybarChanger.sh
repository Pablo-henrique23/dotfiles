...
CONFIG_DIR="$HOME/dotfiles/.config/waybar/configs/configs"
STYLE_DIR="$HOME/dotfiles/.config/waybar/configs/styles"
...
if [[ -n "$SELECTION" && -f "$CONFIG_DIR/$SELECTION.jsonc" && -n "$STYLE_SELECTION" && -f "$STYLE_DIR/$STYLE_SELECTION.css" ]]; then
    ...
    echo "$STYLE_DIR/$STYLE_SELECTION.css" > $HOME/dotfiles/.config/waybar/current_style.css
